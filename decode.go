...
// that can unmarshal a map themselves.
...
func (e InvalidUnmarshalError) Error() string {
	if e.Type == nil {
		return "map: Unmarshal(nil)"
	}
	if e.Type.Kind() != reflect.Pointer {
		return "map: Unmarshal(non-pointer " + e.Type.String() + ")"
	}
	return "map: Unmarshal(nil " + e.Type.String() + ")"
}

// Unmarshal parses the given string into the provided struct v.
// v must be a pointer to a struct, and its fields should be tagged with `map:"<start>,<end>"`
...
func Unmarshal(data []byte, v any) error {
	...
	for i := 0; i < rv.Elem().NumField(); i++ {
		...
		tag := rv.Elem().Type().Field(i).Tag.Get("map")
